...
\begin{algorithm}
    ...
    \label{alg:renameId}
    \begin{algorithmic}
    \Function{renameId}{$\trm{id}, \trm{nodeId}, \trm{nodeSeq}, \trm{renamedBlocks}$}
        ...
        \State $\trm{newPrior} \gets \trm{priority(firstId)}$
        \State $\trm{newFirstId} \gets \trm{new} \ \trm{Id}(\trm{newPrior}, \trm{newId}, \trm{newSeq}, 0)$
        \State $\trm{newLastId} \gets \trm{new} \ \trm{Id}(\trm{newPrior}, \trm{newId}, \trm{newSeq}, \trm{length})$\\
        ...
        \If{$\trm{firstId} < \trm{id}$ \textbf{and} $\trm{id} < \trm{lastId}$}
            ...
            \State $\trm{newPred} \gets \trm{new} \ \trm{Id}(\trm{newPrior}, \trm{nodeId}, \trm{nodeSeq}, \trm{indexOfPred})$
        ...
        \ElsIf{$\trm{newFirstId} < \trm{id}$ \textbf{and} $\trm{id} < \trm{firstId}$}
            \State $\trm{predOfNewFirstId} \gets \trm{new} \ \trm{Id}(\trm{newPrior}, \trm{nodeId}, \trm{nodeSeq}, -1)$
