...
These tuples has four components:
\begin{enumerate*}
...
As stated previously, the size of identifiers from a dense totally ordered set is variable to be able to represent the intended order between elements.
...
Over the years, several works were presented to address this problem of the variable-size identifiers approach.
In \cite{letia:hal-01248270,zawirski:hal-01248197}, authors design for Treedoc a renaming mechanism to reassign shorter identifiers to elements.
...
Only members of the \emph{core} participate in the consensus leading to renaming whereas every node can update the sequence.
...
In \cite{nedelec_2013_lseq,doi:10.1002/cpe.4108}, authors introduce another approach to limit the growth of identifiers : \emph{LSEQ}.
Its insight consists in using several strategies to select the \emph{priority} value of a new tuple.
These strategies prevent nodes from quickly saturating the space between two existing identifiers and having to increase the size of new ones.
Although the \emph{LSEQ} approach has been designed for \emph{Logoot}, it can also be applied to \emph{LogootSplit}.
