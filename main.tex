...
\keywords{CRDT, real-time collaborative editing, eventual consistency, memory-wise optimisation, performance}
...
To limit bandwidth consumption of the renaming mechanism, we propose a compression technique.
With this compression technique, nodes embed only necessary components to uniquely identify a block instead of whole identifiers in \emph{rename} operations.
% Upon reception of the compressed operation, nodes computes the former state by browsing their current state and the concurrent \emph{remove} operations.
This compression technique allows to reduce the metadata to broadcast to a fixed amount per block.
Additionally, by issuing \emph{rename} operations as soon as the state reaches a given number of blocks, we can set an upper-bound to their size.
...
To transform an \emph{insert} or \emph{remove} operation, nodes use the algorithm described in \autoref{alg:renameId}.
...
\begin{algorithm}
    ...
    \label{alg:renameId}
    \begin{algorithmic}
    \State \Comment $\trm{id}$ is the identifier to rename
    \State \Comment $\trm{renamedBlocks}$ is the former state
    \State \Comment $\trm{nId}$ is $\trm{node~id}$ of the node which issued the \emph{rename} op
    \State \Comment $\trm{nSeq}$ is $\trm{node~seq}$ of the node which issued the \emph{rename} op
...
To rename these concurrently inserted identifiers, they also use \autoref{alg:renameId}.
...
\begin{sloppypar}
Our implementation of LogootSplit and RenamableLogootSplit are available at \url{https://github.com/coast-team/mute-structs}.
...
Its insight consists in combining several strategies to select the \emph{position} value of a new tuple.
These strategies prevent nodes from quickly saturating the space between two existing identifiers and having to increase the size of identifiers.
This approach enables to decrease the growth of identifiers.
...
User behaviour studies, inspired by \cite{ignat:hal-01088815,ignat:hal-01238831}, could be led in the context of real-time collaborative writing to identify this limit.
